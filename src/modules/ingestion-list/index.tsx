...
const DATA_STORE: Record<string, any> = {
    ...
    'mar19-apr1': {
        ...
        totalValid: "0", // 待更新
        resolutionRate: "0%", // 待更新
        pendingExclude: "0", // 待更新
        TICKETS_BY_SYSTEM: [ { name: "China Data Portal", value: 0 }, { name: "China Data Platform", value: 0 } ],
        ISSUE_CATEGORIES: [ { name: "OP-OperationTask", value: 0 }, { name: "Req-Account/Access", value: 0 } ],
        TOP_REQUESTERS: [ { name: "TBD", value: 0 }, { name: "TBD", value: 0 } ],
        AGING_TICKETS: [ { assignee: "TBD", date: "2026-03-25", subject: "待填入的长时效工单...", comment: "待追踪" } ],
        ASSIGNEE_WORKLOAD: [ { name: "Kangsheng Shu", total: 0, details: [{ status: "Closed", value: 0 }, { status: "Resolved", value: 0 }, { status: "In Progress", value: 0 }] }, { name: "Winni Luo", total: 0, details: [{ status: "Resolved", value: 0 }, { status: "Closed", value: 0 }, { status: "In Progress", value: 0 }] } ],
        CI_ASSIGNMENT: [ { ci: "China Data Portal", total: 0, assignees: [{ name: "Kangsheng Shu", val: 0 }, { name: "Winni Luo", val: 0 }] } ],
        EFFICIENCY_STATS: [ { name: "Kangsheng Shu", tickets: 0, totalHours: 0, avgTime: 0, topCategory: "TBD" }, { name: "Winni Luo", tickets: 0, totalHours: 0, avgTime: 0, topCategory: "TBD" } ],
        CATEGORY_TIME_COMPARE: [ { category: 'Account/Access', kangsheng: 0, winni: 0, includes: 'TBD' }, { category: 'Operation Task', kangsheng: 0, winni: 0, includes: 'TBD' } ],
        insight_top_req: "（等待最新的洞察提炼词...）",
        insight_scale: "（等待效率对比结论...）",
        insight_depth: "（等待深潜分析数据...）"
    }
};
...
// 主页面组件
// ==========================================
export const IngestionListPage: React.FC = () => {
    // ---- 真正的 Dashboard 页面状态 ----
    const [loading, setLoading] = useState(true);
    const [isExporting, setIsExporting] = useState(false);
    const [viewKey, setViewKey] = useState<string>('mar4-mar18'); // 当前看板绑定的大脑神经元状态
        ...
        <div id="report-root" className={`min-h-screen ${pageConfig.bgColor} pb-24 font-sans text-slate-800 focus:outline-none relative transition-colors duration-1000`}>
            ...
            <div className="max-w-[1240px] mx-auto px-6 space-y-12">
                ...
                <div className="bg-[#f8fafc] rounded-2xl p-12 border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-4 mb-10"><div className="w-1.5 h-8 bg-[#0a1e40] rounded-full" /><div><h2 className="text-2xl font-bold text-[#0a1e40] tracking-tight">Resolution Efficiency Deep-Dive</h2><p className="text-slate-500 text-sm mt-1">Comparative analysis based on Ticket Category and Actual Resolution Time ({data.titleDate} Raw Data)</p></div></div>
                ...
                <div id="pdf-download-btn" className="flex justify-center pt-8">
            ...
            <div id="agent-copilot-container" className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
                ...
                {/* Floating Button */}
                <button onClick={() => setIsAgentOpen(!isAgentOpen)} className="bg-[#0a1e40] text-white p-4 rounded-full shadow-2xl hover:bg-[#0a1e40]/90 transition-transform transform hover:scale-105 active:scale-95 flex items-center gap-2 group relative">
